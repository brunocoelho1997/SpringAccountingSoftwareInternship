...
public class SupplierTransactionService {
    ...
    public Page<SupplierTransaction> findAllPageableByGenre(PageRequest pageable, String value, String frequency, String typeValue, String subTypeValue, Long supplierId, String dateSince, String dateUntil, String valueSince, String valueUntil, Genre genre) {

        //could receive params to filter de list
        if(value!= null || frequency!=null || typeValue != null || supplierId != null|| dateSince != null|| dateUntil != null|| valueSince != null|| valueUntil != null)
            return filterTransactions(pageable, value, frequency, typeValue, subTypeValue, supplierId, dateSince, dateUntil, valueSince, valueUntil, genre);
        else
            return repository.findAllByGenre(pageable, genre);

    }

    private Page<SupplierTransaction> filterTransactions(PageRequest pageable, String value, String frequency, String typeValue, String subTypeValue, Long supplierId, String dateSince, String dateUntil, String valueSince, String valueUntil, Genre genre) {

        Page<SupplierTransaction> supplierTransactions = null;

        if(value.isEmpty() && frequency.isEmpty() && typeValue.isEmpty() && supplierId == 0 && dateSince.isEmpty()&& dateUntil.isEmpty()&& valueSince.isEmpty() && valueUntil.isEmpty())
            return repository.findAllByGenre(pageable, genre);

        Supplier supplier = supplierService.getSupplier(supplierId);
        Specification<SupplierTransaction> specFilter = SupplierTransactionSpecifications.filter(value, frequency, typeValue, subTypeValue, supplier, dateSince, dateUntil,valueSince, valueUntil, genre);
